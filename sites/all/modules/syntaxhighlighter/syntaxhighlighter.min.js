...
if (Drupal.settings.syntaxhighlighter.tagName) SyntaxHighlighter.config.tagName = Drupal.settings.syntaxhighlighter.tagName;
if (SyntaxHighlighter.config.clipboardSwf !== undefined && SyntaxHighlighter.config.clipboardSwf === null && Drupal.settings.syntaxhighlighter.clipboard) SyntaxHighlighter.config.clipboardSwf = Drupal.settings.syntaxhighlighter.clipboard;
jQuery(function(c) {
	if (Drupal.settings.syntaxhighlighter.useAutoloader) {
		syntaxhighlighterAutoloaderSetup();
		SyntaxHighlighter.all()
	} else SyntaxHighlighter.highlight();
	Drupal.settings.syntaxhighlighter.legacyMode && dp.SyntaxHighlighter.HighlightAll("code");
	Drupal.behaviors.syntaxhighlighter = {
		attach: function(a, d) {
			if (a) {
				var b = c(SyntaxHighlighter.config.tagName, a).get(),
					e = b.length;
				for (i = 0; i < e; ++i) SyntaxHighlighter.highlight(SyntaxHighlighter.defaults, b[i])
			} else SyntaxHighlighter.highlight();
			d.syntaxhighlighter.legacyMode && dp.SyntaxHighlighter.HighlightAll("code")
		}
	}
});