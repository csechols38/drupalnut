...
function _custom_load($files) {
  $tp = drupal_get_path('theme', 'custom');
  $file = '';
...
function custom_field($variables) {
  $output = '';
 ...
 */
function custom_field__taxonomy_term_reference($variables) {
  $output = '';
 ...
 */
function custom_links__topbar_main_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
  $links = menu_tree_output(menu_tree_all_data(variable_get('menu_main_links_source', 'main-menu')));
  $output = _custom_links($links);
 ...
 * Formats links for Top Bar http://foundation.zurb.com/docs/components/top-bar.html
 */
function custom_links__topbar_secondary_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
 ...
 *
 * @see custom_links__system_main_menu()
 * @see custom_links__system_secondary_menu()
 */
function _custom_links($links) {
  $output = '';

  foreach (element_children($links) as $key) {
    $output .= _custom_render_link($links[$key]);
  }
 ...
 * @see _custom_links()
 */
function _custom_render_link($link) {
  $output = '';
  ...
  if (!empty($link['#href'])) {
    ...
    if (!isset($rendered_link)) {
      $rendered_link = theme('custom_menu_link', array('link' => $link));
    }
    ...
    if (!empty($link['#below'])) {
      ...
      foreach ($link['#below'] as $sub_link) {
        if (!empty($sub_link['#href'])) {
          $sub_menu .= _custom_render_link($sub_link);
        }
      }
 ...
 */
function theme_custom_menu_link($variables) {
  $link = $variables['link'];
...
function custom_preprocess_html(&$variables) {
  ...
  //  $font = str_replace(' ', '+', theme_get_setting('custom_font'));
  //  if (theme_get_setting('custom_font')) {
   ...
   *   Use with caution.
   */
  // drupal_add_js('document.write(\'<script src=/' . drupal_get_path('theme', 'custom') .'/js/vendor/\'
...
function custom_preprocess_page(&$variables) {
  ...
  // Top bar.
  if ($variables['top_bar'] = theme_get_setting('custom_top_bar_enable')) {
    $top_bar_classes = array();

    if (theme_get_setting('custom_top_bar_grid')) {
      $top_bar_classes[] = 'contain-to-grid';
    }

    if (theme_get_setting('custom_top_bar_sticky')) {
    ...
    $variables['top_bar_menu_text'] = theme_get_setting('custom_top_bar_menu_text');

    $top_bar_options = array();

    if (!theme_get_setting('custom_top_bar_custom_back_text')) {
      $top_bar_options[] = 'custom_back_text:false';
    }

    if ($back_text = theme_get_setting('custom_top_bar_back_text')) {
      if ($back_text !== 'Back') {
        $top_bar_options[] = "back_text:'{$back_text}'";
      }
    }

    if (!theme_get_setting('custom_top_bar_is_hover')) {
      $top_bar_options[] = 'is_hover:false';
    }

    if (!theme_get_setting('custom_top_bar_scrolltop')) {
  ...
  // Messages in modal.
  $variables['custom_messages_modal'] = theme_get_setting('custom_messages_modal');
  ...
  if(theme_get_setting('custom_messages_modal')) {
    drupal_add_js(drupal_get_path('theme', 'custom') . '/js/behavior/reveal.js');
  }
}
...
function custom_css_alter(&$css) {
  ...
  if (theme_get_setting('custom_disable_core_css')) {
 ...
 */
function custom_pager($variables) {
  $tags = $variables['tags'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    if (theme_get_setting('custom_pager_center')) {
...
function custom_theme() {
  ...
  $return['custom_reveal'] = array(
    ...
    'function' => 'theme_custom_reveal',
  );

  $return['custom_menu_link'] = array(
    'variables' => array('link' => NULL),
    'function' => 'theme_custom_menu_link',
  );
  return $return;
}
 ...
 *
 * @see theme_custom_reveal()
 */
function _custom_reveal($reveal = NULL) {
 ...
 *
 * @see custom_theme()
 * @see custom_preprocess_region()
 * @see _custom_reveal()
 */
function theme_custom_reveal($variables) {
  // Generate unique IDs.
  static $counter = 0;
 ...
 */
function _custom_add_reveals() {
  $markup = '';

  // Retrieve reveal markup from static storage.
  foreach (_custom_reveal() as $reveal) {
...
function custom_theme_registry_alter(&$theme_registry) {
  ...
  foreach ($entity_info as $entity => $info) {
    if (isset($entity_info[$entity]['fieldable']) && $entity_info[$entity]['fieldable']) {
      ...
      if (isset($theme_registry[$entity])) {
        $theme_registry[$entity]['preprocess functions'][] = 'custom_entity_variables';
      }
    }
  }
  ...
  if (isset($theme_registry['file_entity'])) {
    $theme_registry['file_entity']['preprocess functions'][] = 'custom_entity_variables';
  }
 ...
 *
 * @see custom_theme_registry_alter()
 */
function custom_entity_variables(&$vars) {
 ...
 */
function custom_process_html_tag(&$vars) {
  if (theme_get_setting('custom_html_tags')) {
    $el = &$vars['element'];
